...
do
		...
		mpiwait=$(pprof -s | grep  "MPI_Wait()" | head -1  | tr -s " " | cut -d " " -f 2)
		echo "size : "$np" , allgatherv : "$allgatherv" , MPI_Isend : "$mpiisend" , MPI_Irecv : "$mpiirecv" , MPI_Wait : "$mpiwait  >> columnPlot_with_opt
done
